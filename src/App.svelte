...
{:else}
  <div class="flex flex-col h-screen overflow-hidden bg-surface-950 text-white font-sans">
    <!-- Navigation Bar -->
    <NavBar bind:currentChannel bind:autoMode {isLoggedIn} {userDisplayName} {currentChannelDisplayName} {timeRemaining} liveCount={liveStreamers.length} onSkip={handleSkip} onLogin={handleLogin} onLogout={handleLogout} onOpenSettings={() => TwitchService.SetSettingsOpen(true)} />
