...
<header class="flex items-center h-14 px-4 bg-surface-900 border-b border-surface-700 select-none">
  ...
  <div class="w-full max-w-xl ml-6 mr-3">
  ...
  <!-- Live Count Badge -->
  {#if isLoggedIn}
    <span class="text-xs text-surface-400 font-sans whitespace-nowrap select-none">
      {i18n.t('liveCount', { count: liveCount })}
    </span>
  {/if}

  <!-- Spacer to push login/settings to the right -->
